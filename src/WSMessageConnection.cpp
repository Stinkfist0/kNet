...
WSMessageConnection::WSMessageConnection(Network *owner, NetworkServer *ownerServer, Socket *socket, ConnectionState startingState)
:MessageConnection(owner, ownerServer, socket, startingState),
tcpInboundSocketData(64 * 1024)
{
}

WSMessageConnection::~WSMessageConnection()
{
	if (owner)
		owner->CloseConnection(this);
}

MessageConnection::SocketReadResult WSMessageConnection::ReadSocket(size_t &totalBytesRead)
{
	...
	{
		LOG(LogVerbose, "WSMessageConnection::ReadSocket: Read throttled! Application cannot consume data fast enough.");
	...
	while(totalBytesRead < maxBytesToRead)
	{
		...
		if (buffer->bytesContains > tcpInboundSocketData.ContiguousFreeBytesLeft())
		{
			...
			if (buffer->bytesContains > tcpInboundSocketData.ContiguousFreeBytesLeft())
			{
				...
				LOG(LogWaits, "WSMessageConnection::ReadSocket: Performance warning! Resized the capacity of the receive ring buffer to %d bytes to accommodate a message of size %d (now have %d bytes of free space)",
					tcpInboundSocketData.Capacity(), buffer->bytesContains, tcpInboundSocketData.ContiguousFreeBytesLeft());
			}
		}

		LOG(LogData, "WSMessageConnection::ReadSocket: Received %d bytes from the network from peer %s.", 
...
MessageConnection::PacketSendResult WSMessageConnection::SendOutPacket()
{
	AssertInWorkerThreadContext();
	...
	{
		LOG(LogVerbose, "WSMessageConnection::SendOutPacket: Socket is not write open %p!", socket);
...
//	assert(ContainerUniqueAndNoNullElements(outboundQueue)); // This precondition should always hold (but very heavy to test, uncomment to debug)
	while(outboundQueue.Size() > 0)
	{
		...
		if (!overlappedTransfer)
		{
			...
			if (!overlappedTransfer)
			{
				LOG(LogError, "WSMessageConnection::SendOutPacket: Starting an overlapped send failed!");
...
//		assert(ContainerUniqueAndNoNullElements(outboundQueue));
		...
		LOG(LogError, "WSMessageConnection::SendOutPacket() failed: Could not initiate overlapped transfer!");

		return PacketSendSocketFull;
	}

	LOG(LogData, "WSMessageConnection::SendOutPacket: Sent %d bytes (%d messages) to peer %s.", (int)writer.BytesFilled(), (int)serializedMessages.size(), socket->ToString().c_str());
...
void WSMessageConnection::DoUpdateConnection() // [worker thread]
{
	ExtractMessages();
}

void WSMessageConnection::SendOutPackets()
{
	AssertInWorkerThreadContext();
...
void WSMessageConnection::ExtractMessages()
{
	AssertInWorkerThreadContext();
	...
	{
		LOG(LogError, "WSMessageConnection::ExtractMessages() caught a network exception: \"%s\"!", e.what());
		if (socket)
			socket->Close();
		connectionState = ConnectionClosed;
	}
...
void WSMessageConnection::PerformDisconnection()
{
...
void WSMessageConnection::DumpConnectionStatus() const
{
	AssertInMainThreadContext();
...
unsigned long WSMessageConnection::TimeUntilCanSendPacket() const
{
	// For WSMessageConnection, this throttling logic is not used. Perhaps will not be used ever, as the
